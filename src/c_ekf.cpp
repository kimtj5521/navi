...
void c_ekf::EKF_Predictionstep(Mat xhat, Mat Phat, Mat odom, double dt)
{
    ...
    A.ptr<double>(0)[0] = 1;  A.ptr<double>(0)[1] = 0; A.ptr<double>(0)[2] = -vel*sin(heading)*dt;
    A.ptr<double>(1)[0] = 0;  A.ptr<double>(1)[1] = 1; A.ptr<double>(1)[2] = vel*cos(heading)*dt;
...
void c_ekf::EKF_Correctionstep(Mat xhat, Mat Phat, double gps_qual_indicator, bool pos_flag, bool heading_flag, Mat zMeasureSensor)
{
    ...
    if(fabs(zhat.ptr<double>(2)[0] - zMeasureSensor.ptr<double>(2)[0]) > PI){
        if(zhat.ptr<double>(2)[0] - zMeasureSensor.ptr<double>(2)[0] > PI){
            zMeasureSensor.ptr<double>(2)[0] = zMeasureSensor.ptr<double>(2)[0] + 2.0*PI;
        }
        else {
            zMeasureSensor.ptr<double>(2)[0] = zMeasureSensor.ptr<double>(2)[0] - 2.0*PI;
        }
...
double c_ekf::AngDiff(double rad)
{
    ...
    double n;
    double cfmod;
    n = floor((rad+PI)/(2.0*PI));
    cfmod = (rad+PI) - n*(2.0*PI);
