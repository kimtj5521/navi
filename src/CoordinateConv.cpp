...
void CoordinateConv::WGS2UTM(double Latitude, double Longitude)
{
    ...
    // easting
    dUTM_X = FE + k0*WN*(WLA + (1 - WT + WC)*pow(WLA,3)/6	+ (5 - 18*WT + pow(WT,2) + 72*WC - 58*Weps)*pow(WLA,5)/120);

    // northing
    // M(lat0) = 0 so not used in following formula
    dUTM_Y = FN + k0*WM + k0*WN*tan(dLat)*(pow(WLA,2)/2 + (5 - WT + 9*WC + 4*pow(WC,2))*pow(WLA,4)/24
...
void CoordinateConv::UTM2WGS(double utmx, double utmy, int utmzone)
{
    ...
    dWGS84_Lon = (Delt*(180/PI)) + Szt;
    dWGS84_Lat = (latr + (1 + e2cuadrada*(cos(latr)*cos(latr)) - 1.5*e2cuadrada*sin(latr)*cos(latr)*(TaO - latr))*(TaO - latr))*(180/PI);
}
*/
